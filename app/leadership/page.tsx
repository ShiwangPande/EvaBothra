    ...
    <main className="min-h-screen bg-white">
      <section className="max-w-6xl mx-auto px-6 md:px-8 py-16 md:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr,1fr] gap-10 lg:gap-16 items-start">
          <div>
            ...
            {data.stats && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6">
                {data.stats.map((s) => (
                  <div key={String(s.label)} className="border border-rose-200 rounded-md p-4">
                    <div className="text-2xl md:text-3xl font-semibold text-rose-950">{s.value}</div>
                    <div className="text-xs uppercase tracking-wider text-rose-700 mt-1">{s.label}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div>
            <div className="relative w-full h-64 md:h-96 rounded-md overflow-hidden border border-rose-200">
