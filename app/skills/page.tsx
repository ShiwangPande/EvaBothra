...
export default function SkillsPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-white via-cyan-50/20 to-white">
      {/* Hero Section */}
      <section className="max-w-6xl mx-auto px-6 md:px-8 py-12 md:py-16">
        <div className="space-y-6">
          <div className="space-y-3">
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-900 bg-gradient-to-r from-cyan-900 to-blue-700 bg-clip-text text-transparent">
              {data.title}
            </h1>
          ...
          {data.stats && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 pt-4">
              {data.stats.map((s, idx) => (
                <div 
                  key={String(s.label)} 
                  className="relative group bg-white border-2 border-cyan-100 rounded-lg p-4 hover:border-cyan-300 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
                  style={{ animationDelay: `${idx * 100}ms` }}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-cyan-50 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  <div className="relative">
                    <div className="text-2xl md:text-3xl font-bold text-cyan-900 mb-1">
                      {s.value}
                    </div>
                    <div className="text-xs uppercase tracking-wider text-cyan-600 font-medium">
                      {s.label}
                    </div>
                  </div>
                </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 pb-16">
        <div className="flex flex-col gap-6">
          {data.items.map((item, idx) => (
            ...
            >
              {/* Gradient Accent */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-500 via-blue-500 to-cyan-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
              ...
              {item.imageSrc && (
                ...
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-cyan-900/20 to-transparent z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  <Image 
                    src={item.imageSrc} 
                    alt={item.title} 
                    fill 
              ...
              <div className="p-5 md:p-6 flex flex-col flex-1">
                ...
                <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                  <h3 className="text-lg md:text-xl font-bold text-gray-900 leading-snug group-hover:text-cyan-900 transition-colors">
                    {item.title}
                  </h3>
                  {item.grade && (
                    <span className="text-xs font-semibold bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-3 py-1 rounded-full whitespace-nowrap shadow-sm">
                      {item.grade}
                    </span>
                  )}
                </div>
                ...
                {item.achievements?.length ? (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {item.achievements.map((a) => (
                      <span 
                        key={a} 
                        className="text-xs font-medium bg-gray-50 border border-gray-200 text-gray-700 px-2.5 py-1 rounded-full hover:bg-cyan-50 hover:border-cyan-200 hover:text-cyan-700 transition-colors"
                      >
                        {a}
                      </span>
                    ))}
                ...
                {item.link && (
                  <a 
                    ...
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-600 hover:text-cyan-800 transition-colors duration-200 group/link w-fit"
                  >
