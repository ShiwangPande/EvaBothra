...
type SkillItem = {
  ...
  details: string;
  achievements: string[];
  imageSrc?: string; // <- ADDED for image
};
        ...
        >
          {(items as SkillItem[]).map((skill) => (
            <motion.div
              ...
              className={`group cursor-pointer relative bg-gradient-to-br ${categoryColors[skill.category]
                ...
                {/* {skill.imageSrc && (
                  <div className="w-full flex justify-center mb-6">
                    <img
                      src={skill.imageSrc}
                      alt={skill.title}
                      className="w-20 h-20 object-cover rounded-2xl shadow-sm border border-slate-200 bg-white"
                    />
                  </div>
                )} */}
                <div className="flex items-start justify-between mb-5">
                  <span
                    className={`text-xs font-bold px-4 py-2 rounded-xl bg-white shadow-md ${categoryTextColors[skill.category]
                      }`}
                  >
                    {skill.category}
                  </span>
          ...
          <div className="fixed inset-0 z-50 overflow-y-auto" aria-hidden={false}>
            ...
            <div className="min-h-screen px-4 py-8 flex items-center justify-center">
              ...
              >
                ...
                <div
                  className={`flex-shrink-0 bg-gradient-to-br ${categoryBgGradients[selectedSkill.category]
                    } px-8 py-8 border-b border-slate-200/50`}
                >
                  <div className="space-y-4 pr-12">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span
                        className={`inline-flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-full ${categoryTextColors[selectedSkill.category]
                          } bg-white shadow-md`}
                      >
                        <span
                          className={`w-2 h-2 rounded-full ${categoryAccentColors[selectedSkill.category]
                            }`}
                        />
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8">
                  {/* Image - best spot just above Overview */}
                  {selectedSkill.imageSrc && (
                    <div className="w-full flex justify-center mb-8">
                      <motion.div
                        ...
                        className="inline-block rounded-3xl shadow-lg bg-gradient-to-br from-slate-50 via-white to-indigo-50 border border-slate-200 p-2"
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={selectedSkill.imageSrc}
                          alt={selectedSkill.title}
                          className="w-32 h-32 md:w-[450px] md:h-[650px] object-cover rounded-2xl transition-transform duration-300 hover:scale-105"
                          style={{
                            boxShadow:
                              "0 8px 28px 0 rgba(63, 81, 181, 0.08), 0 2px 4px 0 rgba(99, 102, 241, 0.1)",
                            background: "white"
                          }}
                        />
                      </motion.div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xl font-bold text-slate-900 flex items-center gap-3">
                      <span
                        className={`w-1.5 h-7 rounded-full ${categoryAccentColors[selectedSkill.category]
                          }`}
                      />
                      Overview
                    </h3>
                  ...
                  <div className="space-y-5 pb-4">
                    ...
                    <div className="space-y-3">
                      {selectedSkill.achievements.map((achievement, index) => (
                        ...
                        >
                          <CheckCircle2
                            className={`flex-shrink-0 w-5 h-5 mt-0.5 ${categoryTextColors[selectedSkill.category]
                              }`}
                          />
